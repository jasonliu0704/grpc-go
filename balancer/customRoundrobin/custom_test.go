...
// all test related to LB stickyness
func (s) TestStickyNess(t *testing.T) {
	r := manual.NewBuilderWithScheme("whatever")
	...
	for i := 0; i < 3*backendCount; i++ {
		ctx := metadata.NewOutgoingContext(context.Background(), metadata.Pairs(OverWriteKeyName, test.addresses[i%backendCount]))
